        ...
        <nav class="flex-1 overflow-y-auto scrollbar-thin px-3 py-5 space-y-6">
            ...
            <div class="space-y-1">
                ...
                <span class="flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold text-gray-400 cursor-not-allowed select-none">
                    <span class="flex items-center gap-3"><i class="fa-solid fa-layer-group w-4 text-center"></i> {{ __('Categorias') }}</span>
                    <span class="text-[10px] font-bold uppercase bg-brand-50 text-brand-400 px-2 py-0.5 rounded-full">{{ __('em breve') }}</span>
                </span>

                <span class="flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold text-gray-400 cursor-not-allowed select-none">
                    <span class="flex items-center gap-3"><i class="fa-solid fa-table-columns w-4 text-center"></i> {{ __('Kanban') }}</span>
                    <span class="text-[10px] font-bold uppercase bg-brand-50 text-brand-400 px-2 py-0.5 rounded-full">{{ __('em breve') }}</span>
                </span>
            </div>
        </nav>
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" @click.outside="open = false"
                    ...
                    <span class="flex-1 min-w-0">
                        <span class="block text-sm font-semibold text-brand-950 truncate">{{ Auth::user()->name }}</span>
                ...
                <div x-show="open" x-cloak x-transition
                    ...
                    <a href="{{ route('profile.edit') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-700">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        ...
                        <button type="submit" class="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-700">
                            <i class="fa-solid fa-right-from-bracket w-4 text-center text-brand-500"></i> {{ __('Sair') }}
                        </button>
                    </form>
                </div>
            </div>
