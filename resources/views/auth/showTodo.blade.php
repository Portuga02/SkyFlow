<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <a href="{{ route('todos.index') }}" class="text-xs font-semibold text-brand-500 hover:text-brand-700">
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-card border border-brand-50 p-6 space-y-4">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @if ($todo->due_date)
                                <span class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold
                                    {{ $todo->is_overdue ? 'bg-rose-100 text-rose-700' : 'bg-brand-50 text-brand-600' }}">
                        ...
                        <div x-data="{ adding: false }" class="flex flex-wrap items-center gap-2">
                            ...
                            <form x-show="adding" x-cloak method="POST" action="{{ route('todos.labels.store', $todo->id) }}"
                                ...
                                <button type="submit" class="px-3 py-2 rounded-lg bg-brand-600 text-white text-xs font-semibold hover:bg-brand-700">{{ __('Add') }}</button>
                            </form>
                        </div>

                        <div class="border-t border-brand-50 pt-4">
                            <span class="text-xs font-semibold uppercase tracking-wide text-brand-400">{{ __('Descrição') }}</span>
                            <p class="mt-1 text-gray-700 leading-relaxed">{{ $todo->description ?: __('Sem descrição.') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-card border border-brand-50 p-6">
                        ...
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="font-bold text-brand-950 flex items-center gap-2">
                                <i class="fa-solid fa-list-check text-brand-500"></i> {{ __('Checklist') }}
                            </h3>
                            <span class="text-xs font-semibold text-brand-500">{{ $progress }}%</span>
                        </div>
                        ...
                        <div class="space-y-2">
                                    ...
                                    <form method="POST" action="{{ route('todos.checklist.toggle', [$todo->id, $i]) }}">
                                        ...
                                        <button type="submit"
                                            class="flex h-6 w-6 items-center justify-center rounded-md border-2 transition shrink-0
                                                {{ ($item['done'] ?? false) ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-brand-300 text-transparent hover:border-brand-500' }}">
                                            <i class="fa-solid fa-check text-[10px]"></i>
                                        </button>
                                    </form>
                                    <span class="flex-1 text-sm text-gray-700 {{ ($item['done'] ?? false) ? 'line-through text-gray-400' : '' }}">
                                        {{ $item['text'] }}
                                    </span>
                            ...
                            @empty
                                <p class="text-sm text-gray-400">{{ __('Nenhum item ainda. Adicione o primeiro passo abaixo.') }}</p>
                            @endforelse
                        </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-card border border-brand-50 p-6">
                        <h3 class="font-bold text-brand-950 flex items-center gap-2 mb-3">
                            <i class="fa-solid fa-paperclip text-brand-500"></i> {{ __('Anexos') }}
                        </h3>
                        ...
                        <div class="space-y-2 mb-4">
                            @forelse ($todo->attachments ?? [] as $i => $file)
                                <div class="flex items-center gap-3 rounded-lg border border-brand-50 px-3 py-2">
                                    ...
                                    <a href="{{ asset('storage/' . $file['path']) }}" target="_blank"
                                        class="flex-1 text-sm text-brand-700 hover:underline truncate">{{ $file['name'] }}</a>
                            ...
                            @empty
                                <p class="text-sm text-gray-400">{{ __('Nenhum anexo enviado ainda.') }}</p>
                            @endforelse
                        </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-card border border-brand-50 p-6">
                        <h3 class="font-bold text-brand-950 flex items-center gap-2 mb-3">
                            <i class="fa-solid fa-comments text-brand-500"></i> {{ __('Comentários') }}
                        </h3>
                                ...
                                <div class="flex gap-3">
                                    ...
                                    <div class="flex-1 bg-brand-50/60 rounded-lg px-3 py-2">
                                        <div class="flex items-center justify-between">
                                            <span class="text-xs font-bold text-brand-900">{{ $comment['user'] }}</span>
                                            <span class="text-[11px] text-gray-400">{{ \Carbon\Carbon::parse($comment['at'])->diffForHumans() }}</span>
                                        </div>
                                        <p class="text-sm text-gray-700 mt-0.5">{{ $comment['body'] }}</p>
                                    </div>
                                </div>
                            @empty
                                <p class="text-sm text-gray-400">{{ __('Seja o primeiro a comentar.') }}</p>
                            @endforelse
                        </div>
                ...
                <div class="space-y-6">
                    <div class="bg-white rounded-2xl shadow-card border border-brand-50 p-6 space-y-4">
                        <h3 class="font-bold text-brand-950 text-sm">{{ __('Detalhes') }}</h3>

                        <div>
                            <span class="text-xs text-gray-400">{{ __('Responsável') }}</span>
                            <div class="flex items-center gap-2 mt-1">
                                @if ($todo->assignee)
                                    ...
                                    <span class="text-sm font-semibold text-brand-950">{{ $todo->assignee->name }}</span>
                                @else
                                    <span class="text-sm text-gray-400">{{ __('Ninguém atribuído') }}</span>
                                @endif
                            </div>
                        </div>
