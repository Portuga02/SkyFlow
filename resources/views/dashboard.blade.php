                                ...
                                @foreach ($urgentTodos as $todo)
                                    <div class="group flex items-start gap-4 p-4 rounded-xl border border-brand-50 hover:border-brand-200 hover:shadow-md transition bg-white">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <a href="{{ route('todos.show', $todo->id) }}" class="block font-bold text-brand-950 truncate hover:text-brand-600 transition">
                                                {{ $todo->title }}
                                            </a>
                                            ...
                                            <div class="flex items-center gap-3 mt-2 flex-wrap">
                                                ...
                                                @if ($todo->category)
                                                    <span class="text-xs font-semibold px-2.5 py-1 rounded-md text-white shadow-sm flex items-center gap-1" style="background-color: {{ $todo->category->color }}">
                                                        <i class="{{ $todo->category->icon ?? 'fa-solid fa-tag' }}"></i> {{ $todo->category->name }}
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
                                ...
                                @foreach ($categories as $category)
                                    <div>
                                        <div class="flex justify-between text-sm font-bold mb-1">
                                            <span class="text-brand-950 flex items-center gap-1"><i class="{{ $category->icon ?? 'fa-solid fa-folder' }} text-gray-400 text-xs"></i> {{ $category->name }}</span>
                                        ...
                                        <div class="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden">
                                            ...
                                            @endphp
                                            <div class="h-2.5 rounded-full transition-all duration-500" style="width: {{ $percent }}%; background-color: {{ $category->color }}"></div>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
