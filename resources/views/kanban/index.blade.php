        ...
        document.querySelectorAll('.kanban-tasks').forEach(column => {
            new Sortable(column, {
                ...
                onEnd: function (evt) {
                    ...
                    const targetColor = evt.to.dataset.color;
    ...
    async function updateCardStatus(cardId, newStatus, columnId) {
        try {
            const response = await fetch('/kanban/mover', {
                ...
                body: JSON.stringify({ id: cardId, status: newStatus, kanban_column_id: columnId })
            });
    ...
    async function quickCreateTask(title, columnId, status) {
        try {
            const response = await fetch('/kanban/tarefa/criacao-rapida', {
                ...
                body: JSON.stringify({ title: title, kanban_column_id: columnId, status: status })
            });
    ...
    function updateCounts() {
        document.querySelectorAll('.kanban-column-container').forEach(container => {
            ...
            if (tasksContainer && badge) {
                badge.textContent = tasksContainer.querySelectorAll('.kanban-card').length;
            }
        });
    }
