<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="text-sm text-brand-600 dark:text-brand-400 mt-1 transition-colors duration-300">{{ __('Escolha um nome, cor e ícone.') }}</p>
        </div>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-card border border-brand-50 dark:border-slate-700 overflow-hidden transition-colors duration-300">
                ...
                <form method="POST" action="{{ route('categories.store') }}" class="p-6 space-y-5"
                    x-data="{ color: '{{ old('color', '#0c8fe6') }}', icon: '{{ old('icon', 'fa-solid fa-layer-group') }}' }">
                    @csrf
                    ...
                    <div>
                        ...
                        <x-text-input id="name" class="block w-full mt-1 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100 dark:placeholder-slate-500 transition-colors" type="text" name="name"
                            placeholder="{{ __('Ex: Trabalho') }}" value="{{ old('name') }}" required autofocus />
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">{{ __('Selecione pra criar esta como uma subcategoria.') }}</p>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        ...
                        <button type="submit"
                            ...
                            <i class="fa-solid fa-floppy-disk"></i> {{ __('Salvar categoria') }}
                        </button>
                    </div>
                </form>
            </div>
